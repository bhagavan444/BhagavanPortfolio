...
function MagneticCursor() {
  ...
  useEffect(() => {
    ...
    const enter = (e) => {
      const t = e.target;
      if (t.closest("a, button, [role='button'], .card-hover")) {
      ...
      <div className="mqinner" style={{
        ...
        {items.map((name, i) => (
          <div key={`${name}-${i}`} style={{ display:"flex", alignItems:"center", gap:"9px", opacity:0.52, flexShrink:0 }}>
            {ICONS[name] && <img src={ICONS[name]} alt={name} className="di" width={20} height={20} loading="lazy"
...
function ExpSection({ data, ri, isLast }) {
  ...
  return (
    <section id={`exp-${data.id}`} ref={ref} data-experience style={{
        ...
        <div className="exp-grid" style={{
          ...
          >
            <div style={{
              ...
              <ML color={C.muted} style={{ marginBottom: mob ? "0.9rem" : "2rem" }}>Impact Metrics</ML>
              ...
              {/* Cert thumbnail — hidden on mobile via CSS */}
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <header ref={heroRef} style={{
          ...
          <p style={{
            ...
            Three industry internships spanning full-stack engineering, computer vision,
            and data science — each with hands-on ownership of real systems, deployed
            models, and production-grade codebases.
          </p>

          {/* Stats — 6 projects corrected */}
        ...
        <section ref={sumRef} style={{
          ...
          <div style={{
            ...
            <ML style={{ marginBottom:"10px" }}>Combined Impact</ML>
            <h2 style={{
              fontFamily:"'Cormorant',Georgia,serif",
          ...
          <div className="sum-grid" style={{
            ...
            {[
              { value:"6",       label:"Projects Shipped",     detail:"Full-stack apps and ML pipelines built end-to-end",       accent:C.accent  },
              { value:"50K+",    label:"Records Processed",    detail:"Structured and text datasets cleaned and modelled",       accent:C.green   },
              { value:"6",       label:"ML Models Evaluated",  detail:"Supervised classifiers trained and benchmarked",          accent:C.purple  },
              { value:"85–90%",  label:"Best Accuracy",        detail:"Image and text classification across held-out test sets", accent:C.accent  },
              { value:"Multiple",label:"API Integrations",     detail:"REST endpoints serving frontend and inference consumers", accent:C.green   },
              { value:"2",       label:"OAuth Providers",      detail:"Google & GitHub auth with JWT session management",        accent:C.purple  },
            ].map((s, i) => (
